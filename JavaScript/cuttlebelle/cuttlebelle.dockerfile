...
# Generator Dir
RUN mkdir /opt/cuttlebelle
COPY . /opt/cuttlebelle/
COPY build.sh benchmark_config.json package.json /opt/cuttlebelle/src/
WORKDIR /opt/cuttlebelle/src

# Instaling Cuttlebelle
RUN yarn
